...
import org.opencloudengine.garuda.model.request.PythonRequests;
...
public class PythonTask extends InterceptorAbstractTask {
    ...
    private PythonRequests pythonRequest;

    @Override
    public void runTask() throws Exception {
        this.pythonRequest = clientJob.getPythonRequest();
        this.clientRequest = (BasicClientRequest) this.pythonRequest;
    ...
    private String buildCommand() throws Exception {
        ...
        //스크립트 파일 파라미터가 있는경우
        if (!StringUtils.isEmpty(pythonRequest.getScriptPath())) {
            buildSingleOption(command, pythonRequest.getScriptPath());
        }
        //스크립트 파일 파라미터가 없는경우
        else {
            saveToFileOption(command, null, pythonRequest.getScript(), workingDir + "/script.py");
        }

        //arguments
        buildArgs(command, pythonRequest.getArguments());
