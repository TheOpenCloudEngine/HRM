...
public class SparkTask extends InterceptorAbstractTask {
    ...
    public void runTask() throws Exception {
        this.sparkRequest = clientJob.getSparkRequest();
        this.clientRequest = (BasicClientRequest) this.sparkRequest;
    ...
    private String buildCommand() throws Exception {
        ...
        buildBasicOption(command, "--deploy-mode", sparkRequest.getDeployMode(), "cluster");

        //--class
        buildBasicOption(command, "--class", sparkRequest.getClassName());

        //applicationJar
        buildSingleOption(command, sparkRequest.getApplicationJar());

        //applicationArguments
        buildArgs(command, sparkRequest.getApplicationArguments());

        //--conf
        buildMapToMultipleOption(command, "--conf", sparkRequest.getConf());

        //--name
        buildBasicOption(command, "--name", sparkRequest.getName());

        //--jars
        buildCommaSeparatedOptions(command, "--jars", sparkRequest.getJars());

        //--packages
        buildCommaSeparatedOptions(command, "--packages", sparkRequest.getPackages());

        //--exclude-packages
        buildCommaSeparatedOptions(command, "--exclude-packages", sparkRequest.getExcludePackages());

        //--repositories
        buildCommaSeparatedOptions(command, "--repositories", sparkRequest.getRepositories());

        //--py-files
        buildCommaSeparatedOptions(command, "--py-files", sparkRequest.getPyFiles());

        //--files
        buildCommaSeparatedOptions(command, "--files", sparkRequest.getFiles());

        //--properties-file
        buildBasicOption(command, "--properties-file", sparkRequest.getPropertiesFile());

        //--driver-memory
        buildBasicOption(command, "--driver-memory", sparkRequest.getDriverMemory());

        //--driver-java-options
        buildBasicOption(command, "--driver-java-options", sparkRequest.getDriverJavaOptions());

        //--driver-library-path
        buildBasicOption(command, "--driver-library-path", sparkRequest.getDriverLibraryPath());

        //--driver-class-path
        buildBasicOption(command, "--driver-class-path", sparkRequest.getDriverClassPath());

        //--executor-memory
        buildBasicOption(command, "--executor-memory", sparkRequest.getExecutorMemory());
        ...
        //--keytab
        buildBasicOption(command, "--keytab", sparkRequest.getKeytab());
