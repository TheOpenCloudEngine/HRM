...
import org.opencloudengine.garuda.model.request.ShellRequests;
...
public class ShellTask extends InterceptorAbstractTask {
    ...
    private ShellRequests shellRequest;

    @Override
    public void runTask() throws Exception {
        this.shellRequest = clientJob.getShellRequest();
        this.clientRequest = (BasicClientRequest) this.shellRequest;
    ...
    private String buildCommand() throws Exception {
        ...
        //스크립트 파일 파라미터가 있는경우
        if (!StringUtils.isEmpty(shellRequest.getScriptPath())) {
            buildSingleOption(command, shellRequest.getScriptPath());
        }
        //스크립트 파일 파라미터가 없는경우
        else {
            saveToFileOption(command, null, shellRequest.getScript(), workingDir + "/shell.sh");
        }

        //arguments
        buildArgs(command, shellRequest.getArguments());
