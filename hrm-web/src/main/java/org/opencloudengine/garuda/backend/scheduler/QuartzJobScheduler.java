...
public class QuartzJobScheduler implements JobScheduler {
    ...
    public JobKey startJob(String jobName, String jobGroupName, String cronExpression, Map<String, Object> dataMap) throws ServiceException {
        try {
            ...
            JobDetail job = JobBuilder.newJob(DefaultClientJob.class).withIdentity(jobKey).build();
        ...
        try {
            ...
            for (String groupName : scheduler.getJobGroupNames()) {
                for (JobKey jobKey : scheduler.getJobKeys(GroupMatcher.jobGroupEquals(groupName))) {
                    ...
                    ClientJob clientJob = (ClientJob) jobDetail.getJobDataMap().get(JobVariable.CLIENT_JOB); // clientJob

                    if (clientJob != null) {
                        job.put("groupName", jobKey.getGroup());
                        job.put("jobName", jobDetail.getJobDataMap().get(JobVariable.JOB_NAME));
                        job.put("jobKey", jobDetail.getJobDataMap().get(JobVariable.JOB_KEY));
                        job.put("jobId", jobDetail.getJobDataMap().get(JobVariable.JOB_ID));
                        job.put("jobType", jobDetail.getJobDataMap().get(JobVariable.JOB_TYPE));
